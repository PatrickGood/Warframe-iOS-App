//
//  PrimaryCllectionCollectionViewController.m
//  Warframe Codex
//
...
static NSString * const reuseIdentifier = @"PrimaryCell";
PrimaryDetailViewController * DetailView8 = nil;
-(NSString *) filePath {
...
- (void)viewDidLoad {
    self.navigationItem.title= @"Primarys";
    [super viewDidLoad];

}
- (void) viewWillAppear:(BOOL)animated{
    [self openDB];
    NSString *sql = [NSString stringWithFormat:@"SELECT * FROM Weapons Where Type = 'Primary'"];
  ...
  /*          char *field9 = (char *) sqlite3_column_text(statement,8);
            ...
            */
            ...
            [[PrimaryStore defaultStore] createName: field1Str
                                             Impact: field2Str
                                           Puncture: field3Str
                                              Slash: field4Str
                                 CriticalMultiplier: field5Str
                                     CriticalChance: field6Str
                                            Descrip: field7Str
                                               Type: field8Str];
            NSLog(@"Primary Name : %@  Rarity: %@", field1Str, field2Str);
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.Primary = [[[PrimaryStore defaultStore] allPrimarys] objectAtIndex:[indexPath row]];
    
    DetailView8.Primary = self.Primary;
    [self performSegueWithIdentifier:@"DetailViewPrimary" sender:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ( [[segue identifier] isEqualToString:@"DetailViewPrimary"] ) {
        
        DetailView8 = [segue destinationViewController];
        DetailView8.Primary = self.Primary;
...
#warning Incomplete method implementation -- Return the number of items in the section
    NSLog(@"%lu",(unsigned long)[[[PrimaryStore defaultStore] allPrimarys] count]);
    return [[[PrimaryStore defaultStore] allPrimarys] count];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Primarys *Primary = [[[PrimaryStore defaultStore] allPrimarys]
                           objectAtIndex:[indexPath row]];
    cell.Label.text = Primary.name;
    NSString * imageName = [NSString stringWithFormat:@"%@%@", Primary.name, @".png"];
    NSLog(@"%@", imageName);
    cell.Image.image = [UIImage imageNamed:imageName];    //UILabel *label = (UILabel *) [cell viewWithTag:100];
    //label.text = Primary.PrimaryName;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (viewControllers.count > 1 && [viewControllers objectAtIndex:viewControllers.count-2] == self	) {
        ...
        [[PrimaryStore defaultStore] clearStore];// Remove all items in store array.
    }
